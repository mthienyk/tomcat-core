...
- `listSignals` et `listUpcomingEvents` renvoient `[]` tant qu’il n’existe pas de board dédié avec des colonnes convenues.
- **Avec l’équipe** : définir le board (signaux, événements), les colonnes, puis brancher le connecteur sur ces données réelles.
